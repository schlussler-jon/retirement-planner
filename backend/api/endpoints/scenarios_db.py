...
def create_scenario(
    ...
    if existing:
        raise HTTPException(status_code=409, detail=f"Scenario '{scenario.scenario_id}' already exists")
    ...
    return {"scenario_id": scenario.scenario_id, "scenario_name": scenario.scenario_name, "message": "Scenario created successfully"}
...
def get_scenario(scenario_id: str, request: Request, db: Session = Depends(get_db)):
    ...
    user_id = get_current_user_id(request)
    db_scenario = db.query(ScenarioModel).filter_by(user_id=user_id, scenario_id=scenario_id).first()
    if not db_scenario:
...
def update_scenario(scenario_id: str, scenario: Scenario, request: Request, db: Session = Depends(get_db)):
    """Update an existing scenario."""
    user_id = get_current_user_id(request)
    if scenario_id != scenario.scenario_id:
        raise HTTPException(status_code=400, detail="Scenario ID in path must match scenario ID in body")
    db_scenario = db.query(ScenarioModel).filter_by(user_id=user_id, scenario_id=scenario_id).first()
    if not db_scenario:
        raise HTTPException(status_code=404, detail="Scenario not found")
    db_scenario.data = scenario.model_dump_json()
    db.commit()
    return {"scenario_id": scenario.scenario_id, "scenario_name": scenario.scenario_name, "message": "Scenario updated successfully"}
