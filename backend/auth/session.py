...
class InMemorySessionStore:
    ...
    def __init__(self):
        self.store: Dict[str, Dict] = {}  # key -> {value, expires_at}
    ...
    def get(self, key: str) -> Optional[str]:
        """Get key value, returning None if expired."""
...
def create_session(user: GoogleUser, access_token: str) -> str:
    ...
    Note: The access_token parameter is accepted for API compatibility but
    is intentionally NOT stored in the session. The app no longer uses
    Google Drive, so persisting live OAuth tokens in Redis is unnecessary
    data exposure. If Drive access is re-added in the future, reconsider
    this decision and ensure tokens are stored with appropriate scoping.

    Args:
        user: Google user information
        access_token: OAuth access token (not stored — see note above)
