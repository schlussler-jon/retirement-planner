    ...
    <div className="bg-slate-900 border border-violet-900 rounded-xl p-5 hover:border-violet-700 transition-colors">
      ...
      {quick && (
        <>
          ...
          {/* Deficit callout — shown when plan has a cumulative shortfall */}
          {quick.financial_summary.total_surplus_deficit < 0 && (
            <Link
              to={`/scenarios/${sc.scenario_id}/results`}
              className="flex items-start gap-2.5 mt-3 px-3 py-2.5 rounded-lg bg-amber-950/40 border border-amber-700/40 hover:border-amber-600/60 transition-colors group"
            >
              <span className="text-amber-400 text-sm shrink-0 mt-0.5">⚠</span>
              <div className="flex-1 min-w-0">
                <p className="font-sans text-amber-300 text-xs font-semibold leading-snug">
                  This plan has a cumulative spending shortfall
                </p>
                <p className="font-sans text-amber-500/80 text-xs mt-0.5 leading-relaxed">
                  Expenses exceed income in some months. View Results to see where the gaps occur and explore adjustments to income, spending, or accounts.
                </p>
              </div>
              <span className="font-sans text-amber-600 group-hover:text-amber-400 text-xs shrink-0 mt-0.5 transition-colors">→</span>
            </Link>
          )}
        </>
      )}
