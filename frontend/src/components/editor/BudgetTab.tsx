...
export default function BudgetTab({ budget, onChange, autoAdd, onAutoAddDone }: Props) {
  ...
  return (
    <div>
      ...
      {budget.categories.length > 0 && (
        <div className="hidden sm:flex items-center gap-2.5 mb-2 ml-6">
          ...
          <span className="w-36 font-sans text-slate-400 text-xs uppercase tracking-wider hidden lg:block">End Date</span>
        </div>
      )}
          ...
          <div key={idx} className="py-3 first:pt-0">
            ...
            <div className="sm:hidden">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  ...
                  />
                  <span className={`font-sans text-sm font-medium ${cat.include ? 'text-white' : 'text-slate-400'}`}>
                    {cat.category_name || cat.main_category || 'New Category'}
                  </span>
                </div>
                <button
                  onClick={() => removeCategory(idx)}
                  className="font-sans text-slate-400 hover:text-red-400 text-lg leading-none transition-colors px-1"
                >×</button>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2 mb-2">
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-slate-800 border border-slate-700 rounded-lg px-2 py-2 text-white font-sans text-sm cursor-pointer focus:border-gold-600 focus:outline-none"
                  >
            ...
            <div className="hidden sm:flex items-center gap-2.5">
              ...
              <input
                ...
                className="w-36 shrink-0 bg-slate-800 border border-slate-700 rounded-lg px-2 py-1.5 text-white font-sans text-sm focus:border-gold-600 focus:outline-none hidden lg:block"
              />
              <button
                onClick={() => removeCategory(idx)}
